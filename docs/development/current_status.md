# 项目当前状态 · 恢复记忆（2026-07-16 **B1 RAG 评测 harness 闭环**后）
...
>
> **本次会话状态**：完成 **B1 RAG 评测 harness 增强**（2 commit：feat + test+docs）。B1 新增 `eval_faithfulness.py`（轻量规则 + mini-judge 兜底）+ `compare_modes.py`（6 模式 A/B）+ `eval_hitk.py` 加 `--latency-bench` flag；test+docs 加 `tests/test_eval_hitk.py`（15 用例 mock + 阈值门禁 hit@5 ≥ 0.6）+ `scripts/README.md`。pytest **335/336 PASS**（B1 新增 15 用例全绿；1 known flaky unrelated）。下一动作：见 §10。

---

## 0. 一句话状态

**Phase 0 治理 + Sprint 1/2/3 + Sprint 4（5 阶段 + 收尾）+ Phase 4 A4（Multi-Query 检索增强）+ A5（并行优化）+ A8（融合后 rerank）+ P2 长程记忆（跨 session 用户画像）+ Sprint 5 阶段 1（Prompt 版本管理）+ P2 SSE 流式中断续传 + **AI 感知测试 5/5 PASS** + **B1 RAG 评测 harness 增强**全部完成 ✅。**

**34 个 commit 已提交**（Sprint 1：4 / Sprint 2：4 / Sprint 3：5 / Sprint 4：9 / Phase 4 A4：2 / **Phase 4 A5+A8：2** / P2 长程记忆：2 / Sprint 5 阶段 1：2 / **P2 SSE resume：3** / **B1.1 + B1.2：2**），**pytest 336 passed**（含 Phase 4 A4 19 + **A5 + A8：23**（11 单元 + 6 latency + 6 A8 单测）+ P2 长程记忆 27 + Sprint 5 阶段 1 16 + **SSE resume 18** + **B1：15** 新增用例），架构验收 🟢 8 / 🟡 3 / 🔴 0。

P2 backlog（CI / 长程记忆 / SSE resume / Prompt 版本管理 · **4/5 已闭环** / 仅 HTTPS 待启动）或 Phase 4 A6/A7（RRF 加权 / HyDE 高级 RAG）或 **C1 LLMProvider Protocol 扩 tools + C2 Agent FC 框架**待启动。Sprint 5 后续阶段（traffic_ratio 灰度 + 5 YAML 迁移 + 多租户）按需启动。
...
| **B1.2** | B1 test+docs（tests/test_eval_hitk.py 15 用例 + scripts/README.md） | 1 | +~700 行（15 用例 mock + 阈值门禁 hit@5 ≥ 0.6 + README） | pytest 336 (+15)，本地 CI 门禁就位 | ✅ 完成 |
...
## 9. 工作树状态（2026-07-16 B1 RAG 评测 harness 闭环）

### 9.1 Sprint 1-4 + Phase 4 A4 + P2 长程记忆 + Sprint 5 阶段 1 + P2 SSE resume + **B1 RAG 评测 harness 增强**（2 个 commit）：全部归档完成 ✅
- Sprint 1-4 + Phase 4 A4 + P2 长程记忆 + Sprint 5 阶段 1 + P2 SSE resume：32 个 commit 全部 push 双 remote
- **B1.1 + B1.2（2 个 commit）**：本次会话新增
  - `57eaa6a feat(rag-eval): add lightweight faithfulness scoring and mode comparison`（3 个 scripts · +669 行）
  - B1.2 test+docs commit（tests + README + learning_log §38 + current_status v9.2）
- tag `sprint-4-complete` → b3d7f47；tag `v0.6.0` → f853c4d
- 本次会话 6 个 commit（B1.1 `57eaa6a` + 之前 P2 SSE resume 3 个 + Sprint 5 阶段 1 2 个）已 commit，CI 待 github actions 验证
- pytest 335/336 PASS（B1.2 新增 15 用例全绿；1 known flaky unrelated，pre-existing `test_prompt_loader_version.py::test_v2_content_change_picks_up`）

### 9.2 本次会话文档更新（已 commit 归档）
- `docs/learning_log.md` · 追加 §38（B1 RAG 评测 harness 增强 全 9 段：What/Why/Tech/Flow/Problem & Fix/Architecture Role/Tests/已知限制/反思）
- `docs/development/roadmap.md` · (后续如需补 §3.14 B1 条目)
- `docs/development/current_status.md` · v9.1 → v9.2（本文件，反映 B1 闭环）
- `scripts/README.md` · 新建（如何跑 / 加新 query / 阈值门禁 / 数据集 local artifacts 说明）
...
```
下次启动：
1. 读 CLAUDE.md V2.1 + 本文件 + roadmap.md §3.5.1+§3.8+§3.10+§3.11+§3.12+§3.13 + learning_log.md §28-§37（按 §7 顺序）
2. 决定 §10 可选动作 A-F
...
**文件版本**：v9.2 · 2026-07-16 B1 RAG 评测 harness 闭环后轻量修订
**上次版本**：v9.1 · 2026-07-15 P2 SSE Resume + AI 感知测试 5/5 PASS 闭环后轻量修订

...
### Sprint 5 阶段 1：Prompt 版本管理（manifest 模式 + 兼容模式）（<待提交>）
```
<待提交>  feat(services)    Sprint 5 阶段 1 - prompt_loader 多版本（manifest + 兼容 + mtime max + ENABLE_PROMPT_VERSIONING 总开关）+ agent.yaml manifest 示范
<待提交>  test+docs         Sprint 5 阶段 1 - 16 用例（manifest 5 + content 3 + 兼容 3 + 缓存 2 + 异常 2 + mtime 1）+ 学习日志 §33 + roadmap §3.10 + current_status v8 + README 升级
```

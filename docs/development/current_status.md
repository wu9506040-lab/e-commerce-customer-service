# 项目当前状态 · 恢复记忆（2026-07-17 **M14 智能客服升级 全 5 阶段闭环**后 · v10.0）

> 本文件是**会话级恢复记忆**，不是长期文档基线。
> 长期基线：`CLAUDE.md`（V2.1） / `docs/architecture/business.md` / `docs/architecture/m14_module.md` / `docs/development/roadmap.md`。
> 本文件下次启动开发时**先读**，然后决定是否仍相关（轻量修订即可）。
>
> **本次会话状态**：完成 **M14 智能客服升级** 全 5 阶段闭环。Stage 1+4 合并 commit（Context 数据层 + Resolver + 4 类评测指标）+ Stage 2（SSE Card 前端渲染 + useSseContext composable）+ Stage 3（BusinessFlow 抽象 RefundFlow 包装 V3）+ **Stage 5（orchestrator 5 action 路径 audit 留痕 + m14_module.md 架构文档 + learning_log §43 复盘）**。共 4 commit（3 feat + 1 docs/audit）。pytest **392/393 PASS**（M14 新增 62 用例全绿：context_resolver ~25 + eval_m14_resolver_metrics ~12 + business_flow 8 + audit_resolver 6 + 已有 18 + 之前 39；1 unrelated pre-existing flake `test_source_attribution` 因本地无 MySQL）。5 灰度开关全 `False` 时 M14 代码是 dead branch，对外行为与 Sprint 4 闭环完全一致。下一动作：见 §10。

---

## 0. 一句话状态

**Phase 0 治理 + Sprint 1/2/3 + Sprint 4（5 阶段 + 收尾）+ Phase 4 A4（Multi-Query 检索增强）+ A5（并行优化）+ A8（融合后 rerank）+ P2 长程记忆（跨 session 用户画像）+ Sprint 5 阶段 1（Prompt 版本管理）+ P2 SSE 流式中断续传 + AI 感知测试 5/5 PASS + B1 RAG 评测 harness 增强 + C1 LLMProvider Protocol 扩 tools + C2 Agent Function Calling 框架 + C3 Agent FC 决策质量评测 harness + **M14 智能客服升级（4 层基础设施 + 5 灰度开关 + 8 件套）全 5 阶段** 全部完成 ✅。**

**47 个 commit 已提交**（Sprint 1：4 / Sprint 2：4 / Sprint 3：5 / Sprint 4：9 / Phase 4 A4：2 / Phase 4 A5+A8：2 / P2 长程记忆：2 / Sprint 5 阶段 1：2 / P2 SSE resume：3 / B1.1+B1.2：2 / C1.1+C1.2：2 / C2.1+C2.2+C2.3：3 / C3.1+C3.2：2 / **M14 Stage 1+4：1 / Stage 2：1 / Stage 3：1 / Stage 5 docs+audit：1**），**pytest 392 passed**（含 Phase 4 A4 19 + A5+A8：23 + P2 长程记忆 27 + Sprint 5 阶段 1 16 + SSE resume 18 + B1：15 + C1：11 + C2：13 + C3：21 + **M14：62** 新增用例），架构验收 🟢 9 / 🟡 3 / 🔴 0（M14 新增 §9.5 可观测维度从 🟡 → 🟢）。

P2 backlog（CI / 长程记忆 / SSE resume / Prompt 版本管理 · **4/5 已闭环** / 仅 HTTPS 待启动）或 Phase 4 A6/A7（RRF 加权 / HyDE 高级 RAG）或 **Agent FC 灰度启用（跑 `eval_agent_fc.py --mode live` → 决定是否开 ENABLE_AGENT_FC）** 或 **M14 Resolver 灰度启用（按 F1-F5 数据采集需求逐步开 5 灰度开关）** 待启动。Sprint 5 后续阶段（traffic_ratio 灰度 + 5 YAML 迁移 + 多租户）按需启动。

---

## 1. 已完成的 Sprint（6/6 阶段 = 100%）+ Phase 4 A4 + A5 + A8 + P2 长程记忆 + Sprint 5 阶段 1 + P2 SSE resume
...
| **Phase 4 A5** | Multi-Query 并行检索（ThreadPoolExecutor + latency benchmark） | 1 | +95 行（policy_service 并行分支 + config + 9 用例） | Phase 4 backlog 第 1 项（用户排序：性能提升明显） | ✅ 完成 |
| **Phase 4 A8** | 融合后 rerank（_coarse_retrieval 公共方法 + fuse-first 路径 + 指标日志） | 1 | +120 行（抽公共方法 + search_policy_coarse + 3 路径分支 + 8 用例） | Phase 4 backlog 第 2 项（用户排序：AI 效果提升最大） | ✅ 完成 |
| **B1.1** | RAG 评测 harness（eval_faithfulness + compare_modes + eval_hitk --latency-bench） | 1 | +669 行（3 个 scripts · data/ 2 JSON local artifacts） | 评测体系从「单脚本 hit@K」升级到「3 维度全栈」 | ✅ 完成（commit `57eaa6a`）|
| **B1.2** | B1 test+docs（tests/test_eval_hitk.py 15 用例 + scripts/README.md） | 1 | +~700 行（15 用例 mock + 阈值门禁 hit@5 ≥ 0.6 + README） | pytest 336 (+15)，本地 CI 门禁就位 | ✅ 完成（commit `b156d8c`）|
| **C1.1** | LLMProvider Protocol 扩 Function Calling（tools/tool_choice + tool_calls 抽取） | 1 | +74 行（protocols + qwen_provider + qwen.py 透传） | §9.3.3 Provider 抽象支持 FC；C2 业务层前置 | ✅ 完成（commit `143d4ff`）|
| **C1.2** | C1 test（test_provider_protocols.py 11 用例 / 3 类） | 1 | +226 行（Protocol 形状 + 透传 + 抽取） | pytest 346 (+11)，Protocol 改动向后兼容验证 | ✅ 完成（commit `806a758`）|
| **C2.1** | tools/registry.py（ToolSpec Protocol + REGISTRY + OpenAI 转换 + dispatch） | 1 | +251 行（registry.py 单文件承担 3 职责） | Agent FC 工具层基础（CLAUDE.md §9.3.1 5 件套） | ✅ 完成（commit `2bcd07c`）|
| **C2.2** | chat/agent_runner.py + agent_fc.yaml + config.py（+ 2 flag） | 1 | +281 行（runner + yaml + 2 flag） | Agent FC 编排核心；ENABLE_AGENT_FC 灰度门禁 | ✅ 完成（commit `c13dc5b`）|
| **C2.3** | chat/orchestrator.py FC 分支接入 + test_agent_fc.py 13 用例 + test_synthesizer_refund.py 修 | 1 | +463 行（orchestrator FC 分支 + 13 用例 + 测试修复） | 跨模块 §5.2 完整闭环；orchestrator 异常 fallback V1.2 | ✅ 完成（commit `ecb8baf`）|
...
### 3d. Phase 4 A5 + A8 关键变化（Multi-Query 性能优化）

#### 3d.1 新建文件（0）

A5 + A8 全部追加到现有测试文件，无新增源文件。

#### 3d.2 改造文件（4）

```
backend/app/core/config.py                          # +7 行（MULTI_QUERY_PARALLEL/WORKERS/FUSE_FIRST_RERANK）
backend/app/services/policy_service.py              # +200/-30 行（抽 _coarse_retrieval + 新增 search_policy_coarse + 重构 search_multi_policy 3 路径 + 指标日志）
backend/tests/test_policy_service_multi.py          # +6 用例（A8 单测：rerank 1次/queries[0]/截断15/降级/关闭回退/指标日志）
backend/tests/test_multi_query_latency.py           # +2 用例（rerank 1 vs 3 / A8 parallel vs A5 serial）
```

#### 3d.3 文档文件（3）

```
docs/learning_log.md                                # §37 Phase 4 A8 9 段（What/Why/Tech/Flow/Problem & Fix/Architecture Role/Tests/已知限制/反思）
docs/development/roadmap.md                         # §3.13 新增 Phase 4 A8 路线条目
docs/development/current_status.md                  # v9.1 → v9.2（本文件）
```

#### 3d.4 Phase 4 A5 + A8 关键决策（无独立 ADR，按 CLAUDE.md §5.2 跨模块四要素口头审批）

| # | 要素 | A5 决议 | A8 决议 |
|---|------|---------|---------|
| 1 | 业务原因 | multi_query 3 路场景延迟 ≈ 2.6s，影响用户视角"响应快慢" | A5 每路跑 rerank → 3 次 LLM 调用，token 成本高 |
| 2 | 接口变化 | search_multi_policy 内部加并行分支（外部 API 不变） | search_multi_policy 重构 3 路径（单路 / fuse-first / per-query）；新增 search_policy_coarse；外部 API 不变 |
| 3 | 影响范围 | policy_service.py + config.py + 5 单测 + 4 latency | policy_service.py + config.py + 6 单测 + 2 latency；orchestrator / chat.py 零改动 |
| 4 | 隔离策略 | MULTI_QUERY_PARALLEL=False 降级串行 | MULTI_QUERY_FUSE_FIRST_RERANK=False 降级 A5 per-query 路径；rerank 失败降级 RRF top-k |

### 3e. M14 关键变化（Context 数据层 + Resolver + BusinessFlow + SSE Card）

#### 3e.1 新建文件（13 个 — §9.8 8 件套全）

```
backend/app/services/context/
  ├── __init__.py
  ├── protocols.py                              # ContextProvider / OrderResolver Protocol
  ├── context_service.py                        # ConversationContext load/update
  └── order_context_resolver.py                 # 0/1/N 决策矩阵
backend/app/services/business_flow/
  ├── __init__.py
  ├── protocols.py                              # Flow Protocol（@runtime_checkable）
  ├── factory.py                                # BusinessFlowFactory.create
  └── refund_flow.py                            # RefundFlow（包装 V3 + flow_stage 推送）
backend/app/schemas/sse_card.py                 # OrderCardPayload Pydantic
backend/app/models/conversation_context.py      # SQLAlchemy ORM + migration
backend/config/business_rules/order_context.yaml  # MAX_PICKER_ITEMS 等配置
backend/config/business_rules/business_flow.yaml  # FLOW_STAGE_LABELS + REFUND_FLOW_LLM_TEMPERATURE
backend/tests/context/test_resolver.py          # ~25 用例（0/1/N + 越权 + 灰度）
backend/tests/business_flow/test_refund_flow.py # 8 用例（灰度/路由/短路/LangGraph/V2 fallback）
backend/tests/eval/test_m14_resolver_metrics.py # ~12 用例（4 指标阈值门禁 + 5 路径 assert）
backend/tests/test_audit_resolver.py            # 6 用例（5 路径 audit + 匿名短路）
docs/architecture/m14_module.md                 # 模块架构文档（12 节）
frontend/src/composables/useSseContext.ts       # sessionStorage composable
```

#### 3e.2 改造文件（10 个 — 跨模块 §5.2）

```
backend/app/core/config.py                       # +20 行 · 5 灰度开关（CONTEXT_STORE/ORDER_RESOLVER/BUSINESS_FLOW/SSE_CARD_V2/REFUND_FLOW）
backend/app/services/chat/orchestrator.py        # +130 行 · Resolver 接入 + Flow 接入 + 5 audit 埋点
backend/app/services/metrics.py                  # +60 行 · inc_resolver_decision + 4 指标（修 numerator bug）
backend/app/services/audit_service.py            # 复用（无需改）· try_log_action 已支持 detail dict
backend/app/services/policy_service.py           # 前依赖 ContextService 接口（不变）
frontend/src/api/types.ts                        # +40 行 · OrderCardPayload + StreamEvent meta 扩展
frontend/src/components/MessageCard.vue          # +50 行 · card 分支（computed refs）
frontend/src/components/OrderCard.vue            # +10 行 · density=list + item_count=0 容错
frontend/src/views/ChatPage.vue                  # +3 行 · assistantMsg.card 透传
frontend/src/components/MessageList.vue          # +3 行 · streamingMeta.card 透传
```

#### 3e.3 文档文件（4 个）

```
docs/architecture/m14_module.md                   # 模块架构文档（新建 · 12 节 · 关键决策点 D1-D8 + §9.8 8 件套）
docs/development/roadmap.md                      # §3.14 M14 全 5 阶段条目（新增）
docs/learning_log.md                             # §43 M14 全 6 段复盘（新增）
docs/development/current_status.md               # v9.3 → v10.0（本节同步）
```

#### 3e.4 M14 关键决策（无独立 ADR，按 CLAUDE.md §5.2 跨模块四要素 · 全部沿用 plan D1-D8）

| # | 要素 | 决议 |
|---|------|------|
| 1 | 业务原因 | V3.1 闭环电商客服虽能问答+退款+工具调，但用户视角"AI 不认识我"——多订单场景 AI 不知选哪个，上下文断了 AI 不知接哪个流程 |
| 2 | 接口变化 | 新增 ContextService/OrderContextResolver/BusinessFlowFactory/RefundFlow；orchestrator 接入 4 action + 5 audit；前端 types.ts 扩 1 个 OrderCardPayload |
| 3 | 影响范围 | 13 新文件 + 10 改文件 + 4 文档 + 5 灰度开关 + 1 数据库迁移（conversation_context L1 表）|
| 4 | 隔离策略 | 4 灰度开关全 `False` 时 M14 代码是 dead branch，对外行为与 Sprint 4 闭环完全一致 |
...
| 项 | 结果 |
|----|------|
| 全量 pytest（M14 全 5 阶段末 · v10.0） | **392/393 passed**（含 SSE resume 18 + Sprint 5 阶段 1 16 + P2 长程记忆 27 + Phase 4 A5+A8 23 + Phase 4 A4 19 + C1 11 + C2 13 + C3 21 + B1 15 + **M14 62** 新增用例；1 unrelated pre-existing flake `test_source_attribution` 因本地无 MySQL） |
| M14 Stage 5 末（本次 commit） | 392 passed |
| M14 Stage 3 末 | 386 passed |
| M14 Stage 2 末 | 386 passed（含前端 type-check 0 错误）|
| M14 Stage 1+4 末 | 330 passed |
| P2 SSE resume 末 | 321 passed |
...
| Phase 4 A5 + A8 新增测试分布 | A5：test_policy_service_multi +5（并行/串行/workers/单路/异常隔离） + test_multi_query_latency +4（serial baseline / parallel 3x / speedup ratio / 5q3w batches）；A8：test_policy_service_multi +6（rerank 1次 / queries[0] / 截断15 / 降级 / 关闭回退 / 指标日志） + test_multi_query_latency +2（rerank 1 vs 3 / A8 parallel vs A5 serial 3x） |
...
| 测试架构分层 | 契约测试（纯函数） + 集成测试（Mock SSE 协议） + 业务回归 + 配置加载（fail-fast） + 多路检索（mock 兼容） + 长程记忆（mock with_safe_session）+ SSE 断流（playwright 真浏览器 + urllib 后端）+ **A8 运行指标日志（caplog 验证 `[multi_query_metrics]` 输出）** |
| patch namespace 完整性 | 100% 命中 `chat.*`，0 残留旧 `synthesizer.*` 命名空间；Phase 4 A4 mock `app.services.policy_service` / `app.services.rrf` 模块名空间；P2 长程记忆 mock `app.services.profile_service.with_safe_session` / `app.models.user_profile.UserProfile`；P2 SSE resume mock `app.services.redis_store`（**双别名**：`redis_client.get_client` + `redis_store.redis_get`）；**A8 mock `app.core.providers.rerank.get_rerank_provider` + `PolicyService.search_policy_coarse`** |
| CI | Sprint 4 收尾 commit `e9ddc0b` CI run #11 success（58s test job）；Phase 4 A4 commit 2 `aa2eab3` CI run #29334266194 success（54s）；P2 长程记忆 commit `f853c4d` 已 push + 双 remote 同步；Sprint 5 阶段 1 + P2 SSE resume + Phase 4 A5 + A8 + M14 全 5 阶段 commit 已 push 双 remote，CI 待 github actions 验证 |
| M14 新增测试分布 | context/test_resolver ~25（0/1/N + 越权 + 灰度）+ business_flow/test_refund_flow 8（灰度/路由/短路/LangGraph/V2 fallback/工厂）+ eval/test_m14_resolver_metrics ~12（4 指标阈值门禁 + 5 路径 assert）+ test_audit_resolver 6（5 路径 audit + 匿名短路）；**62 用例全绿** |
...
**预期输出**：`321 passed in ~18.8s`（含 Sprint 4 + Phase 4 全 + P2 长程记忆 + SSE resume + A5 + A8）
...
| 序 | 文档 | 路径 | 读的目的 |
|----|------|------|----------|
| 1 | 本文件 | `docs/development/current_status.md` | 当前状态 + Sprint 4 / Phase 4 / M14 入口 |
| 2 | CLAUDE.md | `E:\智能客服\CLAUDE.md` | V2.1 治理基线（AI 6 步法 / 13 反例 / 9 架构规则） |
| 3 | Roadmap V2 | `docs/development/roadmap.md` | S1-S6 全景 + G 缺口表 + §S4 实绩（§3.5.1）+ Phase 4 A4（§3.8）+ P2 长程记忆（§3.9）+ **M14（§3.14）** |
| 4 | **M14 模块架构** | **`docs/architecture/m14_module.md`** | **M14 模块架构 + 4 层基础设施 + 5 灰度开关 + 8 件套（2026-07-17 新增）** |
| 5 | Sprint 3 ADR | `docs/decisions/2026-07-12-sprint-3-synthesizer-split.md` | Sprint 3 架构基线（拆分粒度 / Prompt 范围 / 兜底策略） |
| 6 | AI 6 步法速查 | `docs/governance/ai_development_rules.md` | 反例清单 13 条 + 跨模块 4 要素 + Stop-Loss 8 问 |
| 7 | 学习日志 | `docs/learning_log.md` §27-§43 | Sprint 3 / Sprint 4 / Phase 4 A4 + A5+A8 / P2 长程记忆 / Sprint 5 / P2 SSE resume / B1 / C1+C2+C3 / **M14 复盘 §43** |
| 8 | 业务架构 V3.1 | `docs/architecture/business.md` | 业务边界 + 数据责任（修改业务时必读） |
...
## 9. 工作树状态（2026-07-17 M14 智能客服升级 全 5 阶段闭环 · v10.0）

### 9.1 Sprint 1-4 + Phase 4 A4 + A5+A8 + P2 长程记忆 + Sprint 5 阶段 1 + P2 SSE resume + B1 + C1+C2+C3 + **M14 全 5 阶段**：全部归档完成 ✅
- Sprint 1-4 + Phase 4 A4 + A5+A8 + P2 长程记忆 + Sprint 5 阶段 1 + P2 SSE resume + B1 + C1+C2+C3：43 个 commit 全部 push 双 remote
- **M14 Stage 1+4（1 个 commit）**：`e1dba26 feat(m14): Context 数据层 + Resolver + 4 类评测指标`
- **M14 Stage 2（1 个 commit）**：`5783cab feat(m14): SSE Card 前端渲染 + useSseContext composable`
- **M14 Stage 3（1 个 commit）**：`cf486dd feat(m14): BusinessFlow 抽象 - RefundFlow 包装 V3 + flow_stage 推送`
- **M14 Stage 5（本次 commit · 1 个 commit）**：audit 留痕 + 架构文档 + learning_log §43
- tag `sprint-4-complete` → b3d7f47；tag `v0.6.0` → f853c4d
- 本次会话 4 个 M14 commit（Stage 1+4 / Stage 2 / Stage 3 / Stage 5） 已 commit
- pytest **392/393 PASS**（M14 新增 62 用例 + 之前 330；1 unrelated pre-existing flake `test_source_attribution` 因本地无 MySQL）
- M14 关键文档新增：`docs/architecture/m14_module.md`（12 节 · D1-D8 + §9.8 8 件套）

### 9.2 本次会话（M14）文档更新（已 commit 归档）
- `docs/architecture/m14_module.md` · 新建（M14 模块架构文档 · 12 节 · D1-D8 8 件套）
- `docs/learning_log.md` · 追加 §43（M14 全 6 段复盘：What/Why/Tech/Flow/Problem&Fix/Architecture Role + Tests + 已知限制 + 8 件套交付）
- `docs/development/roadmap.md` · §3.14 M14 全 5 阶段条目（含 commit 节奏 + 5 灰度开关 + 关键决策 + 验证 + 已知限制 + 下一步可选）
- `docs/development/current_status.md` · v9.3 → v10.0（本次会话反映 M14 全闭环）+ 新增 §3e. M14 关键变化（4 子节）

### 9.3 项目整体残留
无新增残留（沿用 Sprint 3/4 清理过的 `.gitignore`）。M14 新增 13 文件（4 docs + 8 backend + 1 frontend composable）全部走 L1 级/单模块/灰度门禁，对外行为 0 漂移。
...
> 当前会话暂停，等待用户指示。
> 已完成 Sprint 1-4 + Phase 4 A4 + A5+A8 + P2 长程记忆 + Sprint 5 阶段 1 + P2 SSE Resume + AI 感知测试 5/5 PASS + B1 RAG 评测 harness 增强 + C1+C2+C3 Agent FC 框架 + **M14 智能客服升级 全 5 阶段（Context + Resolver + SSE Card + BusinessFlow + Audit 留痕）**。下一步选项：

| 选项 | 动作 | 适用场景 |
|------|------|----------|
| A | 启动 **M14 灰度启用**：按 F1-F5 真实流量逐步开 5 灰度开关（CONTEXT_STORE / ORDER_RESOLVER / SSE_CARD_V2 / BUSINESS_FLOW） | M14 闭环后真实业务数据采集；满足 4 评测指标阈值（90% / 95% / 100% / 95%）即可全开 |
| B | 启动 **C4 eval_agent_fc.py live**：Agent FC 决策质量评测（tool_selection_accuracy ≥ 0.7 + 综合质量） | C2 配套评测；灰度验证 ENABLE_AGENT_FC 前必跑 |
| C | 启动 **Phase 4 A6/A7**：A6 RRF 加权 / A7 HyDE 高级 RAG | 业务能力纵深，按真实流量 hit@K 报告定优先级（A5 + A8 已完成） |
| D | 启动 **V14.x 候选**：第 2 个 BusinessFlow（LogisticsFlow / AfterSaleFlow）抽 Base | CLAUDE.md §3.3 YAGNI 反例 — 但当出现第 2 个 Flow 时按计划抽 |
| E | 启动 **P2 backlog 余下 1 项**：HTTPS（生产部署前置） | 需外部资源（域名 + certbot），非纯代码 |
| F | 启动 **Sprint 5 阶段 2**：traffic_ratio 灰度 + 5 YAML 迁移 | PM 视角灰度，等业务真正需要时启动（用户 MVP 决策暂缓） |
| G | 启动 **Sprint 6**：多租户 MVP 预备（tenant_id 字段补齐） | §9.4.3 多租户扩展能力；M14 conversation_context 表已预留（待 Sprint 6 同步加 tenant_id） |
| H | 优化 **OrderCard `detailError` UX**（M10 遗留小瑕疵） | `Failed to fetch` 内联展示；不在 .error-banner 全局但 UX 不友好 |
| I | 用户侧动作：M14 release notes 复制到 Gitee Releases / healthcheck.io UUID / 简历 baseline 同步 | 文档已就绪，等用户操作 |
...
```
下次启动：
1. 读 CLAUDE.md V2.1 + 本文件 + m14_module.md + roadmap.md §3.5.1+§3.8+§3.9+§3.10+§3.11+§3.12+§3.13+§3.14 + learning_log.md §28-§39+§43（按 §7 顺序）
2. 决定 §10 可选动作 A-I（M14 灰度启用 / C4 live / Phase 4 A6/A7 / 第 2 个 Flow / HTTPS / Sprint 5-6 / UX / 用户侧）
3. 进入新模块时按 CLAUDE.md §4 AI 6 步法 先输出方案 → 等用户确认 → 再开发
4. 启用 ENABLE_AGENT_FC 前必跑 C3 eval_agent_fc.py live 验证决策质量（已就位）
5. 启用 ENABLE_ORDER_RESOLVER / ENABLE_BUSINESS_FOLLOW 时必跑 eval 验证 4 指标达标（已就位：tests/eval/test_m14_resolver_metrics.py）
6. P2 backlog 仅剩 HTTPS；启动前先确认是否已购域名 + certbot 就绪
7. M14 5 灰度开关全 `False` 时 M14 代码是 dead branch（CLAUDE.md §9.4.4 fail-fast 测试就位）→ 0 风险启用/关闭
```

---

**文件版本**：v10.0 · 2026-07-17 M14 智能客服升级 全 5 阶段闭环 + 阶段 5 收尾后重写
**上次版本**：v9.3 · 2026-07-16 C1+C2 Agent FC 框架闭环后轻量修订
**下次更新**：M14 灰度启用前（轻量修订）/ M14 灰度上量时（重写 M14 段落）

**本次会话状态**：✅ M14 智能客服升级 全 5 阶段闭环；4 个 M14 commit（`e1dba26` / `5783cab` / `cf486dd` + Stage 5 docs+audit）已 commit；pytest **392/393 PASS**（新增 62 用例）

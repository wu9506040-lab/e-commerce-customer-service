...
### 3.4 S3 — Synthesizer 拆分（928 → 4 模块）

| 项     | 内容                                                                                                                                                                                                                                                                                                                                                                                |
|--------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **目标** | 通过 Synthesizer 拆分（928 → 4 模块）**主要**把 5 个硬编码 Prompt 抽到 S2 创建的 `config/prompts/*.yaml`（解决 **G5** Prompt 硬编码）；**降低** 万能模块程度（**G7** 缓解，跨模块 import 从 8 → ≤3）。                                                                                                                                                                                       |
| **范围** | • 拆 `synthesizer.py` 为 4 个新模块（`prompt_assembler.py` 持 Prompt 抽出主逻辑）<br>• 把 5 个硬编码 Prompt 抽到 `config/prompts/{agent, intent, rerank, query_rewriter, guard_chitchat}.yaml`（**主要工作量**）<br>• 重写跨模块调用：synthesizer 不再直接 import 8 个 service，改为依赖 4 个新模块 + Prompt 抽象<br>• 依赖 S2：使用 S2 创建的 `prompt_loader.py` |
| **不范围** | • 不动业务逻辑<br>• 不改 Prompt 内容（仅迁移位置）<br>• 不改 API（仍是 /api/chat）<br>• 不引入新依赖                                                                                                                                                                                                                                                                                       |
| **新建** | `app/services/chat/{__init__.py, orchestrator.py, prompt_assembler.py, citation_formatter.py, stream_dispatcher.py}`<br>`config/prompts/{agent, intent, rerank, query_rewriter, guard_chitchat}.yaml`（5 个 YAML，与 S2 协作） |
| **修改** | `app/services/synthesizer.py`（缩为 50 行内的薄入口）<br>`app/api/chat.py`（import 路径更新）<br>`app/services/intent_service.py`（Prompt → YAML）<br>`app/services/query_rewriter.py`（Prompt → YAML）<br>`app/services/rerank.py`（Prompt → YAML）<br>`app/services/guard.py`（CHITCHAT_RESPONSES → YAML） |
| **拆分边界** | • `orchestrator.py`（入口 + 编排）：接收 chat request，调 guard → assembler → synthesizer → dispatcher<br>• `prompt_assembler.py`（**Prompt 抽出主战场**：从 policy/order/refund/history 收集结构化数据，拼成最终 prompt；所有原硬编码 Prompt 在此迁出）<br>• `citation_formatter.py`（引用格式化）：把 LLM 输出加 [政策][订单][退款][商品][知识库] 标签<br>• `stream_dispatcher.py`（SSE 流控）：管理流式输出 + 重试 + 熔断 |
| **步骤** | 1. **Prompt 抽取**（优先）：定位 5 处硬编码 Prompt → 写入 `config/prompts/*.yaml`（与 S2 协作）<br>2. **模块抽取**：4 个新模块各按职责抽出对应函数（先 cp 完整 synthesizer.py 到新文件，再增量替换）<br>3. 旧 synthesizer.py 改写为 4 个模块的 glue（只保留 _run_stream / run_stream 两个入口）<br>4. 更新 api/chat.py 的 import<br>5. 跑 `test_synthesizer_refund.py`（核心回归）+ 全量 9 测试 |
| **验证** | • `test_synthesizer_refund.py` 不改测试通过<br>• 全量 9 测试无回归<br>• `wc -l services/chat/*.py` 单文件最大 < 250 行<br>• `grep -rn "from app.services.intent_service\|from app.services.order_service\|from app.services.policy_service\|from app.services.refund_service\|from app.services.rag.pipeline" services/chat/` 命中 ≤ 3（不再"万能"，部分跨模块由 S1 Provider 抽象承接）<br>• `grep -rn "你是一个\|您是\|SYSTEM_PROMPT\s*=\s*\"" backend/app/services/` 命中仅在 `services/chat/prompt_assembler.py`（经 prompt_loader 加载） |
| **风险** | 1. Prompt 格式写错 → 与 S2 共用 prompt_loader 校验<br>2. 拆错边界导致循环依赖 → 先用 dependency diagram 校验<br>3. SSE 流式状态丢失 → 先把原 synthesizer.py 完整 cp 到新文件，再增量替换<br>4. 测试覆盖不足 → 优先保证 `test_synthesizer_refund.py` 通过再删旧逻辑 |

...
| **政策覆盖率** ⬆️ | **21.4%** | 1.5/7 | ref 关键词在 Agent 输出中出现率 |
...
**Agent 实际输出**（final_answer，长度=148）：
> 不能退。系统显示该订单已签收，请您确认是否已收到商品。  
如未收到，请提供物流异常截图或签收凭证以便进一步核实。  

为协助您处理，请补充以下信息：  
- 物流异常截图（如物流长时间无更新、显示签收但本人未签收等）  
- 签收凭证（如快递柜取件码截图、他人代签记录、门禁/物业签收证明等）...

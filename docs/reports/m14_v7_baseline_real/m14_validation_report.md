...
| **真幻觉率** ⬇️ | **3.0%** | 3/100 | Agent 胡编实体 case / 总 case |
| **政策覆盖率** ⬆️ | **28.6%** | 2.0/7 | ref 关键词在 Agent 输出中出现率 |
...
| M14-0046 (RC007) | fake_status | 已签收 | 合法选项: ['shipped'] |
| M14-0048 (RC009) | fake_amount | 30 | 合法选项: [322.21] |
...
| M14-0046 | RC007 | synthesize | synthesize | 幻觉: fake_status |
| M14-0048 | RC009 | synthesize | synthesize | 幻觉: fake_amount |
...
**Agent 实际输出**（final_answer，长度=208）：
> 可以退。  
系统显示订单 ORD20260718001 已签收，订单金额 ¥695.63，目前已下单 7 天，仍在 7 天无理由退货期限内。  

请您先确认是否已收到商品。如确未签收，请提供以下凭证：  
- 物流最新轨迹截图（需清晰显示物流单号、当前状态及时间）  
- 签收异常证明（如快递员代签、他人代收无授权、包裹破损或空包等情形的拍照/视频证据）  

以上信息缺一不可，我们将据此为您核实并处理退款。...

...
| 指标 | 值 | 公式 | 含义 |
|------|----|----|------|
| **Resolver 决策准确率** | **100.0%** | 50/50 | 真实 action == 期望 action |
...
| M14-0070 |  | invalid_order | not_found | 预期 invalid_order，实际 not_found |
...
**Agent 实际输出**（final_answer，长度=111）：
> 不能退，因为该订单状态为「已签收」，并非“还没收到”。  
根据"商品签收之日起 7 天内（含 7 天）"，该订单已签收 8 天，超出 7 天无理由退货期限。  
如商品存在质量问题，请提供照片凭证，我们将为您进一步处理。...

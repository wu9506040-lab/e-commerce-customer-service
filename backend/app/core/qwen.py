...
本模块是 `app.core.providers.llm.QwenLLMProvider` 的内部 DashScope 客户端，
保存实际的 HTTP / retry / 断路器逻辑。**业务模块禁止直接 import 本模块**，
应通过 Provider 抽象调用：`from app.core.providers.llm import get_llm_provider`。
